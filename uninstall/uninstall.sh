...
#die starteinträge zum ändern der Device overtlay Einträge löschen
#grep -i "dtoverlay=w1-gpio,gpiopin=4,pullup=on" /boot/config.txt
#if [ $? -eq 0 ]; then sed -i '/^dtoverlay=w1-gpio/d' /boot/config.txt
#else
#echo ""
#fi

#grep -i "w1-gpio pullup=1" /etc/modules
#if [ $? -eq 0 ]; then sed "/^w1-gpio pull/d" /etc/modules
#sed -i "/^w1-ther/d" /etc/modules
#else echo ""
#echo ""
#fi
