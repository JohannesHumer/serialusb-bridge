...
time.sleep(50) # warten bis raspberry fertig gestartet hat
		...
		try:   											#überwachung ob fehler sind											#daten von Arduino Nano
			while True :
				...
				if usberkennung == "USB-3=":	# wenn usb-2 vorgestellt ist die restlichen daten seriell schreiben
					...
					logging.info("DATENWEG USB3 OK UDP ->USB daten  :" + usbdaten)
...
def b():	
		try:   											#überwachung ob fehler sind
			while True :
					...
					response2 = usb3praefix + response					# kopie wird erstellt und mit der präfix befüllt
					sock.sendto(response2,(miniserverIP,virtualUDPPort)) 		#daten werden zu loxone gesendet 
					logging.info("DATENWEG USB3 OK USB -> UDP daten  :" + response2)
		except Exception as e:
