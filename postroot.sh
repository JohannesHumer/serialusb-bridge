...
#die module in die startdateien anfügen
#grep -i "dtoverlay=w1-gpio,gpiopin=4,pullup=on" /boot/config.txt #überprüfen ob die enträge schon vorhanden sind
...
#if [ $? -eq 0 ]; then echo"" #wenn sie schon vorhanden sind nichts tun
#else echo "w1-gpio pullup=1" >> /etc/modules #ansonsten die Einträge einfügen
